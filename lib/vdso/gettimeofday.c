...
int do_hres_timens(const struct vdso_data *vdns, const struct vdso_clock *vcns,
...
#ifdef CONFIG_TIME_NS
static __always_inline
int do_coarse_timens(const struct vdso_data *vdns, const struct vdso_clock *vcns,
		     clockid_t clk, struct __kernel_timespec *ts)
{
	const struct vdso_data *vd = __arch_get_timens_vdso_data(vdns);
	const struct timens_offset *offs = &vcns->offset[clk];
...
static __always_inline
int do_coarse(const struct vdso_data *vd, const struct vdso_clock *vc,
	      clockid_t clk, struct __kernel_timespec *ts)
{
	const struct vdso_timestamp *vdso_ts = &vc->basetime[clk];
	u32 seq;
